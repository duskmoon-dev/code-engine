import { ExternalTokenizer, LRParser } from '../../parser/lr';
import { styleTags, tags } from '../../parser/highlight';
...
const notIn = 189,
  ...
  docAt = 191,
  beforeUnaryOp = 192,
  newlineTerminator = 193,
  newlineBeforeBinaryOperator = 194,
  newlineBeforeDo = 195,
  immediateParenOpen = 1,
  immediateSquareOpen = 2,
  immediateColonWhitespace = 196,
  atomWord = 197,
  keywordWord = 198,
  quotedContentIDouble = 3,
  quotedContentISingle = 4,
  quotedContentIHeredocDouble = 5,
  quotedContentIHeredocSingle = 6,
  quotedContentIParenthesis = 7,
  quotedContentICurly = 8,
  quotedContentISquare = 9,
  quotedContentIAngle = 10,
  quotedContentIBar = 11,
  quotedContentISlash = 12,
  quotedContentDouble = 13,
  quotedContentSingle = 14,
  quotedContentHeredocDouble = 15,
  quotedContentHeredocSingle = 16,
  quotedContentParenthesis = 17,
  quotedContentCurly = 18,
  quotedContentSquare = 19,
  quotedContentAngle = 20,
  quotedContentBar = 21,
  quotedContentSlash = 22,
  SigilModifiers = 23,
  identifier = 199,
  underscoredIdentifier = 200;
...
const quotedContentTokens = new ExternalTokenizer((input, stack) => {
  ...
  for (let hasContent = false; true; hasContent = true) {
    ...
    if (input.next === info.endDelimiter) {
      ...
      if (input.next === CP_HASH) {
        ...
        if (isHeredoc && input.peek(1) === CP_NEWLINE) ; else if (
...
const sigilTokens = new ExternalTokenizer((input, stack) => {
  let length = 0;
...
const immediateTokens = new ExternalTokenizer((input, stack) => {
...
const beforeUnaryOpToken = new ExternalTokenizer((input, stack) => {
...
const atomWordToken = new ExternalTokenizer((input, stack) => {
...
const keywordWordToken = new ExternalTokenizer((input, stack) => {
  let [nextCodepointString, nextOffset] = peekCodepoint(input);
  if (ATOM_START.test(nextCodepointString)) {
    do {
      input.advance(nextOffset);
...
const identifierToken = new ExternalTokenizer((input, stack) => {
  let [nextCodepointString, nextOffset] = peekCodepoint(input);
...
const notInToken = new ExternalTokenizer((input, stack) => {
...
const doubleSlashToken = new ExternalTokenizer((input, stack) => {
...
const atToken = new ExternalTokenizer((input, stack) => {
  if (input.next === CP_AT) {
    input.advance();
...
const newlineTokens = new ExternalTokenizer((input, stack) => {
  if (input.next === CP_CR) {
    input.advance();
  }
...
const elixirHighlighting = styleTags({
