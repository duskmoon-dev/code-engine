import {EditorView, Command, ViewPlugin, PluginValue, ViewUpdate, logException,
        getTooltip, TooltipView} from "../view"
import {Transaction, Prec} from "../state"
...
export const completionPlugin = ViewPlugin.fromClass(class implements PluginValue {
  ...
  update(update: ViewUpdate) {
    ...
    this.debounceUpdate = cState.active.some(a => a.isPending && !this.running.some(q => q.active.source == a.source))
      ? setTimeout(() => this.startUpdate(), delay) as unknown as number : -1
  ...
  startUpdate() {
    ...
    if (this.running.length && cState.open && cState.open.disabled)
      this.debounceAccept = setTimeout(() => this.accept(),
                                       this.view.state.facet(completionConfig).updateSyncTime) as unknown as number
  }
