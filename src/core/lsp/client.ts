...
import {showDialog} from "../view"
import {ChangeSet, ChangeDesc, MapMode, Text, Extension} from "../state"
...
export class LSPClient {
  ...
  connect(transport: Transport) {
    ...
    this.requestInner<lsp.InitializeParams, lsp.InitializeResult>("initialize", {
      processId: null,
      clientInfo: {name: "@duskmoon-dev/code-engine/lsp"},
  ...
  private requestInner<Params, Result>(
    ...
    let req = new Request<Result>(id, params, setTimeout(() => this.timeoutRequest(req), this.timeout) as unknown as number)
