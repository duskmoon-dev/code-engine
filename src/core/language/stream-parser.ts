import {Tree, Input, TreeFragment, NodeType, NodeSet, PartialParse, Parser, NodeProp} from "../../parser/common"
import {Tag, tags as highlightTags, styleTags} from "../../parser/highlight"
...
export class StreamLanguage<State> extends Language {
  ...
  private constructor(parser: StreamParser<State>) {
    ...
    this.topNode = docID(data, this as StreamLanguage<unknown>)
    self = this
    this.streamParser = p
...
function findStartInFragments<State>(lang: StreamLanguage<State>, fragments: readonly TreeFragment[],
  ...
  for (let f of fragments) {
    ...
    if (found && found.pos <= endPos && (tree = cutTree(lang as StreamLanguage<unknown>, f.tree, startPos + f.offset, found.pos + f.offset, false)))
