import {EditorView, ViewPlugin, Decoration, DecorationSet,
        ...
        PanelConstructor, Panel, showPanel, getPanel} from "../view"
import {Text, StateEffect, StateField, Extension, TransactionSpec, Transaction,
        EditorState, Facet, combineConfig, RangeSet, Range, RangeSetBuilder} from "../state"
import elt from "../view/crelt"
...
const lintPlugin = ViewPlugin.fromClass(class {
  ...
  constructor(readonly view: EditorView) {
    ...
    this.timeout = setTimeout(this.run, delay) as unknown as number
  }
  ...
  run() {
    ...
    if (now < this.lintTime - 10) {
      this.timeout = setTimeout(this.run, this.lintTime - now) as unknown as number
    } else {
      this.set = false
  ...
  update(update: ViewUpdate) {
    ...
    if (update.docChanged || config != update.startState.facet(lintConfig) ||
      ...
      if (!this.set) {
        this.set = true
        this.timeout = setTimeout(this.run, config.delay) as unknown as number
      }
    }
  }
...
function batchResults<T>(promises: readonly Promise<T>[], sink: (values: T[]) => void, error: (reason: any) => void) {
  ...
  for (let p of promises) p.then(value => {
    ...
    else timeout = setTimeout(() => sink(collected), 200) as unknown as number
  }, error)
}
