import {EditorState, StateCommand, EditorSelection, SelectionRange,
        ...
        findClusterBreak, Text, Line, countColumn, CharCategory} from "../state"
import {EditorView, Command, Direction, KeyBinding} from "../view"
import {syntaxTree, IndentContext, getIndentUnit, indentUnit, indentString,
        getIndentation, matchBrackets, MatchResult} from "../language"
import {SyntaxNode, NodeProp} from "../../parser/common"
...
function moveBySyntax(state: EditorState, start: SelectionRange, forward: boolean) {
  ...
  for (let at = start.head;;) {
    ...
    if (!next) break
    if (interestingNode(state, next, bracketProp as NodeProp<unknown>)) pos = next
