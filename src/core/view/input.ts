import {EditorSelection, EditorState, SelectionRange, RangeSet, Annotation, Text, Facet} from "../state"
...
class MouseSelection {
  ...
  setScrollSpeed(sx: number, sy: number) {
    ...
    if (sx || sy) {
      if (this.scrolling < 0) this.scrolling = setInterval(() => this.scroll(), 50) as unknown as number
