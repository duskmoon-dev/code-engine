import {EditorState, Transaction, StateEffect, StateEffectType,
        Facet, StateField, Extension, MapMode, FacetReader} from "../state"
...
class HoverPlugin {
  ...
  update() {
    if (this.pending) {
      ...
      this.restartTimeout = setTimeout(() => this.startHover(), 20) as unknown as number
    }
  }
  ...
  checkHover() {
    ...
    if (hovered < this.hoverTime)
      this.hoverTimeout = setTimeout(this.checkHover, this.hoverTime - hovered) as unknown as number
    else
      this.startHover()
  }
  ...
  mousemove(event: MouseEvent) {
    ...
    if (this.hoverTimeout < 0) this.hoverTimeout = setTimeout(this.checkHover, this.hoverTime) as unknown as number
